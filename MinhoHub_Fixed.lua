...
local function LoadAutoConfig()
    ...
    pcall(function()
        if isfile(autoLoadFile) then
            local name = readfile(autoLoadFile):match("^%s*(.-)%s*$")
...
addCheckbox(vizSec1, "Wings", Config.Visuals.Wings, function(v) 
    Config.Visuals.Wings = v 
    ApplyCharacterEffects(player.Character)
end)
addCheckbox(vizSec1, "Halo", Config.Visuals.Halo, function(v) 
    Config.Visuals.Halo = v 
    ApplyCharacterEffects(player.Character)
end)
addCheckbox(vizSec1, "Floor Aura", Config.Visuals.FloorAura, function(v) 
...
addCheckbox(vizSec2, "Yokai Aura", Config.Visuals.YokaiAura, function(v) 
    Config.Visuals.YokaiAura = v 
    ApplyCharacterEffects(player.Character)
end)
addCheckbox(vizSec2, "Toxic Wastelands Horns", Config.Visuals.ToxicWastelandHorns, function(v) 
...
local isMobile = game:GetService("UserInputService"):GetLastInputType() == Enum.UserInputType.Touch
