...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        local function configPath(name) return CONFIG_DIR .. configName(name) end
        local function saveConfig(name)
            if not writefile then return false end
            local filename = configName(name)
            local ok = pcall(writefile, configPath(filename), base:encodeJSON())
            if ok then
                selectedConfig = filename
                if nameBox then nameBox:set(filename) end
                if configList and configList.add then pcall(function() configList:add(filename) end) end
            end
            return ok
        end
        local function loadConfig(name)
            ...
            if isfile(path) then pcall(function() base:decodeJSON(readfile(path)) end) end
        end

        createButton.changed:Connect(function()
            local filename = tostring(nameBox.value or ""):gsub("%s+", "")
            if filename == "" then return end
            filename = configName(filename)
            if isfile and isfile(configPath(filename)) then
                selectedConfig = filename
                if nameBox then nameBox:set(filename) end
                return
            end
        ...
        saveButton.changed:Connect(function()
            saveConfig(nameBox.value ~= "" and nameBox.value or selectedConfig)
        end)
        loadButton.changed:Connect(function()
            loadConfig(nameBox.value ~= "" and nameBox.value or selectedConfig)
        end)
        deleteButton.changed:Connect(function()
            if delfile then pcall(delfile, configPath(nameBox.value ~= "" and nameBox.value or selectedConfig)) end
        end)
        for flag, feature in next, base.features do
            if flag ~= "settings/config/auto_save" and feature.changed and feature.changed.Connect then
                feature.changed:Connect(function()
                    if autoSaveToggle.value then saveConfig(selectedConfig) end
                end)
            end
        end
    end
