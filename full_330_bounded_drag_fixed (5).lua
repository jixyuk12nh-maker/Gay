...
do
    ...
    local ThemeAccentInstances = {}
    local ThemeTextSizeInstances = {}

    local function registerThemeAccent(instance, property)
        property = property or "BackgroundColor3"
        table.insert(ThemeAccentInstances, { instance = instance, property = property })
        instance[property] = Theme.Accent
        return instance
    end

    local function registerThemeTextSize(instance)
        table.insert(ThemeTextSizeInstances, instance)
        return instance
    end

    local function setThemeAccent(color)
        Theme.Accent = color

        for _, entry in ThemeAccentInstances do
            pcall(function()
    ...
    local function setThemeTextSize(size)
        Theme.TextSize = size

        for _, instance in ThemeTextSizeInstances do
            pcall(function()
    ...
    local self = setmetatable({}, UISection)
    	...
    	function UISection._makeInstances(self, side)
    		...
    		registerThemeAccent(theme, "BackgroundColor3")
    ...
    do
    	...
    	function UIColorpickerMenu._makeInstances(self)
    		local container= Instance.new("Frame")
    		registerThemeAccent(container, "BackgroundColor3")
    ...
    assert(self.feature, "")
    	...
    	function UIDropdownMenu._makeInstances(self)
    		...
    		registerThemeAccent(layout, "ScrollBarImageColor3")
    ...
    UIBase.__index = UIBase do
    	...
    	function UIBase._makeInstances(self)
    		...
    		open.TextSize = Theme.TextSize
    		registerThemeTextSize(open)
    ...
    assert(label, "UISlider.setLabel(_, label) : _ -> expected string, got nil")
    	...
    	function UISlider._makeInstances(self, parent)
    		...
    		plus.TextSize = Theme.TextSize
    		registerThemeTextSize(plus)
    		...
    		minus.TextSize = Theme.TextSize
    		registerThemeTextSize(minus)
    ...
    self:_makeInstances(parent)
    		...
    		self._trove:Connect(self.instances.button.InputBegan, function(input)
    			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
    				...
    				self.changed:Fire()
    				self.instances.button.TextColor3 = Theme.Accent
    			end
    		end)
    ...
    self.value = value
    	...
    	function UICurveGraph._makeInstances(self, parent)
    		...
    		for i = 1, 19 do
    			...
    			registerThemeAccent(point, "BackgroundColor3")
...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        do
            ...
            do
                local accent = menu:newLabel("settings/menu/label_accent"):setLabel("Accent"):newColorpicker("settings/menu/accent")
                accent:set({rgb = Theme.Accent, alpha = 1})
                accent.changed:Connect(function(value)
                    setThemeAccent(value.rgb)
                end)

                menu:newLabel("settings/menu/label_font"):setLabel("Font")
                menu:newDropdown("settings/menu/font", false, {"SourceSans"}):set("SourceSans")

                local textSize = menu:newSlider("settings/menu/text_size", 1, 26, 1):set(16):setLabel("Text Size")
                textSize.changed:Connect(function(value)
                    setThemeTextSize(value)
                end)
            ...
            do
                local configName = configuration:newTextBox("settings/config/name"):setLabel("Config Name")
                local createBtn = configuration:newButton("settings/config/create"):setLabel("Create")

                local configList = configuration:newList("settings/config/list", 6, {})

                local loadBtn = configuration:newButton("settings/config/load"):setLabel("Load")
                local saveBtn = configuration:newButton("settings/config/save"):setLabel("Save")
                local deleteBtn = configuration:newButton("settings/config/delete"):setLabel("Delete")

                local autoSaveToggle = configuration:newToggle("settings/config/auto_save"):setLabel("Auto Save To Config")
                local autoSaveDropdown = configuration:newDropdown("settings/config/auto_save_target", false, {})

                local autoLoadToggle = configuration:newToggle("settings/config/auto_load"):setLabel("Auto Load Config")
                local autoLoadDropdown = configuration:newDropdown("settings/config/auto_load_target", false, {})

                local function configPath(name)
                    return HOME_DIR .. "configs/" .. name .. ".json"
                end

                local function addConfigName(name)
                    configList:add(name)
                    autoSaveDropdown:add(name)
                    autoLoadDropdown:add(name)
                end

                local function removeConfigName(name)
                    configList:remove(name)
                    autoSaveDropdown:remove(name)
                    autoLoadDropdown:remove(name)
                end

                if listfiles and isfolder and isfolder(HOME_DIR .. "configs") then
                    for _, path in listfiles(HOME_DIR .. "configs") do
                ...
                createBtn.changed:Connect(function()
                    ...
                    if writefile then
                        writefile(configPath(name), base:encodeJSON())
                    end

                    addConfigName(name)
                    configList:set(name)
                    autoSaveDropdown:set(name)
                    autoLoadDropdown:set(name)
                end)

                loadBtn.changed:Connect(function()
                    local name = configList.value

                    if not name or not readfile or not isfile then
                        return
                    end

                    if isfile(configPath(name)) then
                        base:decodeJSON(readfile(configPath(name)))
                    end
                end)
                ...
                deleteBtn.changed:Connect(function()
                    local name = configList.value
                    ...
                    removeConfigName(name)
                end)

                if writefile then
                    for _, feature in next, base.features do
                        feature.changed:Connect(function()
                            if autoSaveToggle.value and autoSaveDropdown.value then
                                writefile(configPath(autoSaveDropdown.value), base:encodeJSON())
                            end
                        end)
                    end
                end

                if autoLoadToggle.value and autoLoadDropdown.value and readfile and isfile then
                    if isfile(configPath(autoLoadDropdown.value)) then
                        base:decodeJSON(readfile(configPath(autoLoadDropdown.value)))
                    end
                end
            end
        end
