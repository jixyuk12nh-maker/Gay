...
-- Create folders only when they do not already exist.
...
do
        ...
        if writefile then
            for _, feature in next, base.features do
                feature.changed:Connect(function()
                    writefile(CONFIG_DIR .. "main.json", base:encodeJSON())
                end)
            end
        end
    end
    ...
    if readfile and isfile then
        if isfile(CONFIG_DIR .. "main.json") then
            base:decodeJSON(readfile(CONFIG_DIR .. "main.json"))
        end
    end
