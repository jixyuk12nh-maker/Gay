...
do
    ...
    -- Theme system: central accent color / text size state that every
    -- UI element registers into, so changing a Settings value updates
    -- every already-created instance immediately.
    local Theme = {
    ...
    local function themeTrackAccent(instance, property)
    	property = property or "BackgroundColor3"
    	table.insert(ThemeRegistry.accent, { instance = instance, property = property })
    	pcall(function()
    ...
    local function themeTrackTextSize(instance)
    	table.insert(ThemeRegistry.textSize, instance)
    	pcall(function()
    ...
    local function themeSetAccent(color)
    	Theme.Accent = color

    	for i = #ThemeRegistry.accent, 1, -1 do
    		local entry = ThemeRegistry.accent[i]
    		local ok = pcall(function()
    			entry.instance[entry.property] = color
    		end)

    		if not ok then
    			table.remove(ThemeRegistry.accent, i)
    		end
    	end
    end

    local function themeSetTextSize(size)
    	Theme.TextSize = size

    	for i = #ThemeRegistry.textSize, 1, -1 do
    		local instance = ThemeRegistry.textSize[i]
    		local ok = pcall(function()
    			instance.TextSize = size
    		end)

    		if not ok then
    			table.remove(ThemeRegistry.textSize, i)
    		end
    	end
    end
    ...
    local self = setmetatable({}, UISection)
    	...
    	function UISection._makeInstances(self, side)
    		...
    		themeTrackAccent(theme, "BackgroundColor3")
    ...
    do
    	...
    	function UIColorpickerMenu._makeInstances(self)
    		local container= Instance.new("Frame")
    		themeTrackAccent(container, "BackgroundColor3")
    ...
    assert(self.feature, "")
    	...
    	function UIDropdownMenu._makeInstances(self)
    		...
    		themeTrackAccent(layout, "ScrollBarImageColor3")
    ...
    self.value = value
    	...
    	function UICurveGraph._makeInstances(self, parent)
    		...
    		for i = 1, 19 do
    			...
    			themeTrackAccent(point, "BackgroundColor3")
...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        do
            ...
            do
                local accentPicker = menu:newLabel("settings/menu/label_accent"):setLabel("Accent"):newColorpicker("settings/menu/accent"):set({rgb = Theme.Accent, alpha = 1})
                accentPicker.changed:Connect(function(value)
                    themeSetAccent(value.rgb)
                end)

                menu:newLabel("settings/menu/label_font"):setLabel("Font")
                menu:newDropdown("settings/menu/font", false, {"SourceSans"}):set("SourceSans")

                local textSizeSlider = menu:newSlider("settings/menu/text_size", 8, 24, 1):set(Theme.TextSize):setLabel("Text Size")
                textSizeSlider.changed:Connect(function(value)
                    themeSetTextSize(value)
                end)
            ...
            do
                local CONFIG_DIR = HOME_DIR .. "configs/"

                local nameBox = configuration:newTextBox("settings/config/name"):setLabel("Config Name")
                local createButton = configuration:newButton("settings/config/create"):setLabel("Create")
                local configList = configuration:newList("settings/config/list", 6, {})
                local loadButton = configuration:newButton("settings/config/load"):setLabel("Load")
                local saveButton = configuration:newButton("settings/config/save"):setLabel("Save")
                local deleteButton = configuration:newButton("settings/config/delete"):setLabel("Delete")
                ...
                local function refreshConfigList()
                    for option in configList.options do
                        configList:remove(option)
                    end

                    if isfolder and listfiles and isfolder(CONFIG_DIR) then
                        for _, path in listfiles(CONFIG_DIR) do
                    ...
                    if not next(configList.options) then
                        configList:add("default.json")
                    end

                    if not configList.value then
                        for option in configList.options do
                            configList:set(option)
                            break
                        end
                    end
                end
                ...
                createButton.changed:Connect(function()
                    local name = sanitizeConfigName(nameBox.value)
                    if name == "" then
                        return
                    end
                    ...
                    if writefile then
                        writefile(CONFIG_DIR .. name, base:encodeJSON())
                    end
                ...
                saveButton.changed:Connect(function()
                    ...
                    if writefile then
                        writefile(CONFIG_DIR .. target, base:encodeJSON())
                    end
                end)
                ...
                loadButton.changed:Connect(function()
                    ...
                    if readfile and isfile and isfile(CONFIG_DIR .. target) then
                        base:decodeJSON(readfile(CONFIG_DIR .. target))
                    end
                end)
                ...
                deleteButton.changed:Connect(function()
                    ...
                    if delfile and isfile and isfile(CONFIG_DIR .. target) then
                        delfile(CONFIG_DIR .. target)
                    end
