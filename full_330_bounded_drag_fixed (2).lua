...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        local skins = tabList:newTab("Skins")
        local skinsTabs = skins:newTabList()
        do
            local main = skinsTabs:newTab("Main"):intoSections()
            do
                local info = main:newSection("left", "Main")
                do
                    info:newToggle("skins/main/unlock_all"):setLabel("Unlock All")
                    info:newLabel("skins/main/label_info"):setLabel("When unlock all is enabled you can pick any")
                    info:newLabel("skins/main/label_info2"):setLabel("skin in lobby and we will apply it in game")

                    if not getconnections or not islclosure or not getconstants or not setconstant then
                        info:newLabel("skins/main/label_info3"):setLabel("NOT SUPPORTED ON YOUR EXECUTOR!")
                    end
                end
            end

            local skin = skinsTabs:newTab("Skin"):intoSections()
            do
                local selection = skin:newSection("left", "Selection")
                do
                    selection:newToggle("skins/skin/enabled"):setLabel("Enable Skin")
                    selection:newLabel("skins/skin/label_selected"):setLabel("Skin")
                    selection:newDropdown("skins/skin/selected", false, {"Default"}):set("Default")
                end
            end

            local wrap = skinsTabs:newTab("Wrap"):intoSections()
            do
                local selection = wrap:newSection("left", "Selection")
                do
                    selection:newToggle("skins/wrap/enabled"):setLabel("Enable Wrap")
                    selection:newLabel("skins/wrap/label_selected"):setLabel("Wrap")
                    selection:newDropdown("skins/wrap/selected", false, {"Default"}):set("Default")
                end
            end

            local charm = skinsTabs:newTab("Charm"):intoSections()
            do
                local selection = charm:newSection("left", "Selection")
                do
                    selection:newToggle("skins/charm/enabled"):setLabel("Enable Charm")
                    selection:newLabel("skins/charm/label_selected"):setLabel("Charm")
                    selection:newDropdown("skins/charm/selected", false, {"Default"}):set("Default")
                end
            end

            local finisher = skinsTabs:newTab("Finisher"):intoSections()
            do
                local selection = finisher:newSection("left", "Selection")
                do
                    selection:newToggle("skins/finisher/enabled"):setLabel("Enable Finisher")
                    selection:newLabel("skins/finisher/label_selected"):setLabel("Finisher")
                    selection:newDropdown("skins/finisher/selected", false, {"Default"}):set("Default")
                end
            end
        end
