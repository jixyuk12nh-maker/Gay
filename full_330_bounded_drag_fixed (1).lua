...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        do
            ...
            local picker = skins:newSection("left", "Cosmetics")
            do
                local btnSkin = picker:newButton("skins/picker/btn_skin"):setLabel("Skin")
                local btnWrap = picker:newButton("skins/picker/btn_wrap"):setLabel("Wrap")
                local btnCharm = picker:newButton("skins/picker/btn_charm"):setLabel("Charm")
                local btnFinisher = picker:newButton("skins/picker/btn_finisher"):setLabel("Finisher")

                local list = picker:newList("skins/picker/list", 6, {})
                ...
                local function setListItems(items)
                    for option in list.options do
                        list:remove(option)
                    end
                ...
                setListItems(categoryItems.Skin)

                btnSkin.changed:Connect(function()
                    setListItems(categoryItems.Skin)
                end)

                btnWrap.changed:Connect(function()
                    setListItems(categoryItems.Wrap)
                end)

                btnCharm.changed:Connect(function()
                    setListItems(categoryItems.Charm)
                end)

                btnFinisher.changed:Connect(function()
                    setListItems(categoryItems.Finisher)
                end)
            end
        end
