...
do
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        local knownConfigs = { ["default.json"] = true }
        ...
        local function addConfigToList(filename)
            ...
            if not filename or knownConfigs[filename] then return end
        ...
        local function selectedConfigName()
            ...
            return configName(value) or selectedConfig
        end

        local function saveConfig(filename)
            filename = configName(filename) or selectedConfig
        ...
        local function loadConfig(filename)
            filename = configName(filename) or selectedConfig
            if not readfile or not isfile or not isfile(configPath(filename)) then return false end
            local ok = pcall(function()
                base:decodeJSON(readfile(configPath(filename)))
            end)
            if ok then
                selectedConfig = filename
                addConfigToList(filename)
        ...
        createButton.changed:Connect(function()
            local filename = configName(nameBox and nameBox.value)
            if not filename then return end
            -- Create always captures the current UI state, including current text size.
            saveConfig(filename)
        end)
        ...
        deleteButton.changed:Connect(function()
            local filename = selectedConfigName()
            if delfile and isfile and isfile(configPath(filename)) then
                pcall(delfile, configPath(filename))
            end
            knownConfigs[filename] = nil
            if selectedConfig == filename then selectedConfig = "default.json" end
        end)

        if configList and configList.changed and configList.changed.Connect then
            configList.changed:Connect(function(value)
                if type(value) == "table" then value = value[1] end
                local filename = configName(value)
                if filename then selectedConfig = filename end
            end)
        end

        for flag, feature in next, base.features do
            if flag ~= "settings/config/auto_save" and feature.changed and feature.changed.Connect then
                feature.changed:Connect(function()
                    if autoSaveToggle and autoSaveToggle.value then
