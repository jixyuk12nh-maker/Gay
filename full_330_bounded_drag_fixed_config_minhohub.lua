    ...
    local base = UIBase.new():setLabel("Minho Hub") do
        ...
        do
            ...
            do
                ...
                configuration:newDropdown("settings/config/auto_save_config", false, {"default.json"}):set("default.json"):setLabel("Auto Save Config")
                configuration:newToggle("settings/config/auto_load"):setLabel("Auto Load Config")
                configuration:newDropdown("settings/config/auto_load_config", false, {"default.json"}):set("default.json"):setLabel("Auto Load Config")
            end
        end
    
    
        local selectedConfig = "default.json"
        local autoSaveConfig = nil
        local autoLoadConfig = nil

        local function ensureConfigDir()
            if makefolder and not isfolder(CONFIG_DIR) then pcall(makefolder, CONFIG_DIR) end
        end
        ...
        local nameFeature = base.features["settings/config/name"]
        local listFeature = base.features["settings/config/list"]
        ...
        local autoLoadToggle = base.features["settings/config/auto_load"]
        local autoSaveSelect = base.features["settings/config/auto_save_config"]
        local autoLoadSelect = base.features["settings/config/auto_load_config"]

        if autoSaveToggle then
            autoSaveToggle.changed:Connect(function()
                if autoSaveToggle.value == true then
                    autoSaveConfig = (autoSaveSelect and autoSaveSelect.value) or selectedConfig
                else
                    autoSaveConfig = nil
                end
            end)
        ...
        if autoLoadToggle then
            autoLoadToggle.changed:Connect(function()
                if autoLoadToggle.value == true then
                    loadConfig((autoLoadSelect and autoLoadSelect.value) or "default.json")
                end
            end)
        end
        if createButton then
            createButton.changed and createButton.changed.Connect and createButton.changed:Connect(function()
                local name = nameFeature and nameFeature.value or "default"
                saveConfig(name)
                if listFeature then listFeature:set({selectedConfig}) end
            end)
        end
        if saveButton then
            saveButton.changed and saveButton.changed.Connect and saveButton.changed:Connect(function()
                saveConfig((nameFeature and nameFeature.value) or selectedConfig)
            end)
        end
        if loadButton then
            loadButton.changed and loadButton.changed.Connect and loadButton.changed:Connect(function()
                loadConfig((nameFeature and nameFeature.value) or selectedConfig)
            end)
        end
        if deleteButton then
            deleteButton.changed and deleteButton.changed.Connect and deleteButton.changed:Connect(function()
                if delfile then pcall(delfile, configPath((nameFeature and nameFeature.value) or selectedConfig)) end
            end)
        end

        if writefile then
            for flag, feature in next, base.features do
                if feature.changed and feature.changed.Connect then feature.changed:Connect(function()
                    if autoSaveConfig and flag ~= "settings/config/auto_save" then
                        pcall(saveConfig, autoSaveConfig)
                    end
                end) end
            end
        end

        if readfile and isfile and isfile(configPath("default.json")) then
            loadConfig("default")
        end
